...
from DevTools.Ntuplizer.skimValidationParams import params
...
def add_job(jobname,args,name,params):
    ...
    outdir = 'validation_skims/{0}/{1}'.format(jobname,name)
    if not args.full: kwargs['maxEvents'] = 1000
    kwargs['outputFile'] = '{0}_miniTree.root'.format(name)
    if args.save: kwargs['outputFile'] = '{0}/skim.root'.format(outdir)
    config = 'DevTools/Ntuplizer/test/MuMuTauTau_cfg.py'
    ...
    if args.save:
        ...
        if args.bg:
            command += ' > {0}/skim.log 2>&1 &'.format(outdir)
        else:
            command += ' 2>&1 | tee {0}/skim.log'.format(outdir)
    os.system(command)
